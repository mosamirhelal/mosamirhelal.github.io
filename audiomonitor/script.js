const T = {
  en: {
    ...
    micLabel: "Microphone",
    micDesc: "Visualizes audio captured directly from your default microphone.",
...
function applyTheme() {
  ...
  document.getElementById("theme-icon").innerHTML = isDark ? sunSVG : moonSVG;
}
...
function drawCanvas(id, freqData, timeData, cfg) {
  ...
  const vals = [];
  for (let i = 0; i < NUM; i++) {
    vals.push(Math.min(1, (data[i * step] / 255) * Math.min(3, sensitivity)));
  }
...
function animate() {
  ...
  if (micActive && cardVis.mic && micAnalyser) {
    ...
    document.getElementById("mic-db").textContent = db !== null ? db : "∞";
  }
  if (sysActive && cardVis.sys && sysAnalyser) {
    ...
    document.getElementById("sys-db").textContent = db !== null ? db : "∞";
  }
}
...
async function toggleSys() {
  if (!sysActive) {
    try {
      ...
      if (e.name !== "NotAllowedError") alert(T[lang].alertSysErr + e.message);
    }
  } else {
    stopSys();
  }
