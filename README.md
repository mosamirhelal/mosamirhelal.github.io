...
A full school management ecosystem built for **مدرسة الشهيد محمود علي الخطيب الرسمية للغات**, consisting of four interconnected web applications backed by two purpose-built Google Apps Script backends.
...
- **Month** × **Exam Type** × **Subject** → e.g., `تقييم مارس - عربي`, `امتحان أبريل - رياضيات`

The first 3 columns of every sheet are frozen: Student Name, Student ID, and Section. It also auto-creates:
- `المستخدمين` — teacher accounts with username, password, name, status, and allowed subjects
- `سجل الحركات` — full audit log of every operation with timestamp
- `إعدادات النظام` — a settings control panel with **dropdown validations** for month filter, exam type filter, results visibility (show/hide), max score display in reports, and completion status filter
...
- Taa marbouta `ة` → `ه`
- Alef maqsura `ى` → `ي`
- All diacritics (tashkeel) → stripped entirely

This means a parent searching for `محمد` will correctly find `مُحَمَّد`, `محمود`, etc. — no exact spelling required.
...
6. Accounts with status other than `نشط` are rejected even with correct credentials

---

**🎓 Subject-Level Authorization**

Every protected API call verifies that the authenticated teacher's `allowedSubjects` field covers the requested subject. If the teacher's token contains `allowedSubjects = "عربي,رياضيات"` and they request data for `علوم`, the system rejects the request with a specific error message and logs the unauthorized access attempt to the audit log with the teacher's name and the subject they tried to access.

---

**📋 Grade Fetching — `getStudents` action**

Filters the grade sheet by section, locates the exact column matching the combination of `examType + month + subject`, and returns only the students in the requested section along with their existing scores. Every fetch is logged with full context (grade, section, column name).
...
5. **Audit log** — the log entry includes the exact row numbers modified (collapsed into ranges like `5 إلى 9، 14`), the teacher name, subject, grade, and section
...
**By ID (`?id=...`):**
1. Reads system settings to check if results are currently visible (`عرض النتيجة للطلاب = نعم/لا`)
...
4. Reads the **max-grades sheet** (`النهايات العظمى`) to get the out-of score for each subject per section — allowing different sections to have different maximums
...
- **Teacher** (معلم) — can only view their own statistics and personal history
...
**`بيانات المعلمين`** — set up by `setupTeachersSheet()`: teacher code, national ID, name, specialization, scheduled start time, total annual leave balance, and total working days. Start time is stored in `HH:mm` format.

**`شيت العمليات`** — set up by `setupOperationsSheet()`: every attendance event is stored as a row with teacher code, national ID, name, operation type, and the date split into **separate day/month/year columns** (to avoid Google Sheets timezone serial-number conversion issues), plus quantity/minutes, notes, attachment reference, recording admin, and full timestamp.

**`التقارير المجمعة`** — built by `setupAggregatedReports()`: one row per teacher, populated with **live SUMIFS formulas** aggregating directly from the operations sheet. Tracks 9 operation types and computes remaining leave balance and attendance rate. This sheet is the single source of truth for the teacher dashboard charts.
...
1. The backend reads the teacher's row from `التقارير المجمعة` — an instant lookup, not a full scan
...
Rather than calculating stats at query time (slow for large datasets), this function builds a **live formula sheet** once. For each teacher in `بيانات المعلمين`, it inserts a row with static identity fields and **dynamic SUMIFS formulas** for all 9 operation types — e.g., `=SUMIFS('شيت العمليات'!$H:$H, 'شيت العمليات'!$A:$A, $A2, 'شيت العمليات'!$D:$D, "تأخر صباحي")`. It also generates a remaining-leave formula and an attendance-rate formula that reads total working days from the teachers sheet. The result: whenever a new record is added to the operations sheet, **all aggregated numbers update instantly** through Google Sheets' native formula engine — zero backend recalculation needed.
